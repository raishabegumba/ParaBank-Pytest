...
class FindTransactionsPage(BasePage):
    ...
    def navigate_to_find_transactions(self) -> None:
        ...
        try:
            ...
            log.error(f"Navigation to Find Transactions page failed: {e}")
            raise
    ...
    def select_account(self, account_id: str) -> None:
        """Select account (parallel-safe version)."""

    # =========================================================================
    # INPUT METHODS
    # =========================================================================

    def enter_transaction_id(self, transaction_id: str) -> None:
        ...
        self.fill(self.TRANSACTION_ID_FIELD, transaction_id)

        log.info(f"Entered transaction ID: {transaction_id}")
    ...
    def enter_date_range(
        ...
        log.info(
            f"Entered date range: {from_date} - {to_date}"
        )
    ...
    def click_find_transactions(
        ...
        log.info(f"Clicked Find Transactions button: {button}")
    ...
    def wait_for_search_results(
        ...
        try:
            ...
            log.error(f"Search results wait failed: {e}")
            return False
        ...
        try:
            ...
            for i in range(row_count):
                ...
                try:
                    ...
                    transaction = {
                        "transaction_id": (
                            cells.nth(0).text_content() or ""
                        ).strip(),

                        "date": (
                            cells.nth(1).text_content() or ""
                        ).strip(),

                        "description": (
                            cells.nth(2).text_content() or ""
                        ).strip(),
                    ...
                    log.warning(
                        f"Failed to parse transaction row {i}: "
                        f"{row_error}"
                    )
            ...
            log.error(f"Failed to retrieve transactions: {e}")
            return []
    ...
    def has_transactions(self) -> bool:
        """Check whether results contain transactions."""
    ...
    def get_transactions_by_amount_range(
        ...
        for transaction in self.get_transactions():
            ...
            try:
                ...
                log.warning(
                    "Could not parse transaction amount"
                )
    ...
    def validate_search_criteria(self) -> dict:
        ...
        account_value = self.page.locator(
    ...
    def clear_search_form(self) -> None:
        ...
        try:
            ...
            for field in fields:
                if self.is_visible(field, timeout=1000):
                    self.fill(field, "")
            ...
            log.error(f"Failed to clear form: {e}")
    ...
    def assert_transactions_found(
        ...
        assert transaction_count >= min_count, (
            ...
            f"transactions, found {transaction_count}"
        )

        log.info(
            f"Verified {transaction_count} transactions found"
        )

    def assert_no_transactions_found(self) -> None:
        """Assert no transactions found."""

        message = self.get_no_results_message().lower()

        assert (
            "no results" in message
            or "no transactions" in message
        ), "Expected no transactions message not displayed"

        log.info("Verified no transactions found")
        ...
        try:
            export_data = {
                ...
                "search_criteria": {
                    ...
                    "transaction_id": self.get_attribute(
                        self.TRANSACTION_ID_FIELD,
                        "value"
                    ),
            ...
            log.info(
                "Successfully exported transaction data"
            )
            ...
            log.error(
                f"Failed to export transaction data: {e}"
            )
